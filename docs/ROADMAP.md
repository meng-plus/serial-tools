...
- ✅ **协议扩展系统（protocol-ext）**：前端 JS 脚本 + manifest.yaml 免重编译新增协议；内置 Modbus RTU/TCP 主站+从站参考实现（本地主从闭环自测）、模板、演示 zip；协议仪表盘（网格布局）；工作区持久化协议实例。详见 [protocol-ext/](./protocol-ext/README.md)
